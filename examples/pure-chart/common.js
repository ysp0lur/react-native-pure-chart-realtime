  ...
  flattenData.map((series) => {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (typeof dataProp[i] === 'number') {
        simpleTypeCount++
        dataObject.ratioY = (dataProp[i]-0) / (maxClone-0) * height
        dataObject.y = dataProp[i]
        dataObject = {
          gap: i * gap,
          ratioY: (dataProp[i]-0) / (maxClone-0) * height,
          y: dataProp[i]
        }
        ...
        if (typeof dataProp[i].y === 'number' && dataProp[i].x) {
          ...
          dataObject = {
            gap: i * gap,
            ratioY: (dataProp[i].y -0) / (maxClone-0) * height,
            x: dataProp[i].x,
            y: dataProp[i].y,
            isEmpty: isEmpty
          }
...
export const getGuideArray = (max, height, numberOfPoints = 5) => {
  ...
  let incr = (definedMax - 0) / numberOfPoints
    for (let i = 1; i < numberOfPoints + 1; i++) {    
    let  val = 0 + (i*incr)
